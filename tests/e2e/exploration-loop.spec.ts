...
test('completes and reloads an explainable exploration loop', async ({ page }, testInfo) => {
  ...
  await expect(active).toHaveAttribute('data-exploration-status', 'accepted', { timeout: 30_000 })
  ...
  await expect(resumed).toHaveAttribute('data-exploration-status', 'in-progress', { timeout: 30_000 })
  ...
  await expect(restored.locator(`[data-exploration-id="${suggestionId}"]`)).toHaveCount(0, { timeout: 30_000 })
  const history = restored.getByText(/最近决定/)
  await history.click()
  await expect(restored.locator(`[data-exploration-history-id="${suggestionId}"]`))
    .toContainText('核对来源并记录一个结论', { timeout: 30_000 })
  await expectNoHorizontalOverflow(page)
})
