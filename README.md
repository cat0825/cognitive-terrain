...
- 搜索、标签筛选、峰值标签与笔记详情联动。
- 七种可视化口径：密度、熟练度、探索度、近期活跃海拔、学习进程、活动温度和领域。近期活跃海拔（`activity-elevation-v1`）只把近期打开、编辑和复习事件的衰减聚合映射到高度；学习进程（`learning-progression-v1`）只重放带时区的显式认知观测；温度只把同类活动热度编码为颜色，并保持知识密度海拔。它们都不改变稳定语义平面坐标，也不把活动事件当作学习证据。
- 熟练度来自显式 `mastery` 认知状态；学习进度只重放带时区的显式认知观测，并保留自评、YAML 导入、复习结果和迁移来源。打开、编辑乃至单独的复习活动事件都不会提高熟练度。无历史时保留当前快照值但使用中性学习进程海拔并标记高不确定性；旧项目不补造历史。默认 profile 不衰减，只有显式选择版本化衰减 profile 才会随时间改变海拔。
