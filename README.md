...
- `activity-elevation-v1` 使用打开（weight 1，半衰期 7 天）、编辑（3，30 天）、复习（2.5，14 天）的指数衰减；`score = 1 - exp(-rawHeat / 3)`。同一事件 ID 去重，打开事件在 60 秒内只计一次；保留聚合以其首末时间、计数和 compaction heat 参与计算，raw/aggregate provenance 在详情证据中分开显示。非法、未来或不支持的输入会被忽略：晚于评估时间超过 5 分钟时钟偏移容差的事件与聚合一律不参与热度、海拔、温度、最近活动和缺口陈旧判定，导入项目包时会以警告列出被忽略的条数，其余数据照常导入。评估时间固定后结果确定。
...
普通笔记导入会在本地预检后才进入 embedding：单文件最多 4 MiB、单批最多
500 个文件和 32 MiB、最多 2,000 条记录、每条正文最多 64 KiB；解析并发固定为
4 路。`.terrain.json` 项目包上限为 64 MiB。超限会显示实际值和允许值，默认不
会截断或启动分析；用户明确选择“按上限整理”后，才会去重、截断正文并保留上限
内的记录。解析期间可取消，取消不会写入部分项目。

已验证支持范围：1800 条演示笔记可完成完整分析与交互门禁；2,000 条是当前有
明确资源边界的最大导入规模，不代表 10k/50k 数据集已获得性能保证。更大的知识库
应先拆批导入并观察预检中的预计耗时；语义模型预计时间明显高于离线快速模式。
...
CI 形态与成本取舍（分片数、并发取消、纯文档改动跳过浏览器门禁）见 [`docs/ci-cost.md`](docs/ci-cost.md)。
