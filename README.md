...
- 熟练度来自显式 `mastery` 认知状态；学习进度只重放带时区的显式认知观测，并保留自评、YAML 导入、复习结果和迁移来源。打开、编辑乃至单独的复习活动事件都不会提高熟练度。无历史时使用中性海拔并标记高不确定性；旧项目只保留当前快照，不补造历史。默认 profile 不衰减，只有显式选择版本化衰减 profile 才会随时间改变海拔。
